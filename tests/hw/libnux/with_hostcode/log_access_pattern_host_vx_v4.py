...
from dlens_vx_v4.halco import iter_all, PPUOnDLS, SynapseRowOnSynram, \
...
from pyhaldls_vx_v4 import SynapseQuad, Barrier
...
class LibnuxAccessPatternTestVx(unittest.TestCase):
    ...
    def test_refgen_access(self):
        ...
        program = join(TEST_BINARY_PATH,
                       "refgen_access_pattern-ppu_vx_v4.bin")
    ...
    def test_synram_access(self):
        ...
        program = join(TEST_BINARY_PATH,
                       "synram_access_pattern-ppu_vx_v4.bin")
        initial_logsize = os.path.getsize(FLANGE_LOG_PATH)
        log.debug(f"Initial size of {FLANGE_LOG_PATH}: "
                  f"{initial_logsize}bytes.")
    ...
    def test_correlation_reset_causal(self):
        ...
        program = join(TEST_BINARY_PATH,
                       "correlation_reset_causal-ppu_vx_v4.bin")
        initial_logsize = os.path.getsize(FLANGE_LOG_PATH)
        log.debug(f"Initial size of {FLANGE_LOG_PATH}: "
                  f"{initial_logsize}bytes.")
    ...
    def test_neuron_reset_global(self):
        ...
        program = join(TEST_BINARY_PATH,
                       "neuron_reset_global-ppu_vx_v4.bin")
        initial_logsize = os.path.getsize(FLANGE_LOG_PATH)
        log.debug(f"Initial size of {FLANGE_LOG_PATH}: "
                  f"{initial_logsize}bytes.")
