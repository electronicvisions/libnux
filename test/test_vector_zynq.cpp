...
void start()
{
	...
	{
		libnux_testcase_begin("Vector (generated) write to extmem (vector address via vector type), scalar read (generated) of one element");
		auto const ga = GlobalAddress::from_global(AddressSpace::extmem.to_global_omnibus() + next_offset);
		next_offset += 0x100;
		*ga.to_extmem().to_vector<__vector uint16_t>() = vec_splat_u16(0x1);
		auto val = *ga.to_extmem().to_scalar<uint16_t>();
		libnux_test_equal(val, 0x1);
		libnux_testcase_end();
		libnux_test_write_string("at address: ");
		libnux_test_write_string(to_hexstring(ga.to_global_omnibus()).data());
		libnux_test_write_string("\n");
	}

	{
		libnux_testcase_begin("Vector (fxvstax) write to extmem (scalar address), scalar read (generated) of one element");
		auto const ga = GlobalAddress::from_global(AddressSpace::extmem.to_global_omnibus() + next_offset);
		next_offset += 0x100;
	...
	{
		libnux_testcase_begin("Vector (fxvoutx) write to extmem (hioff(scalar) >> 4 | 1<<31), scalar read (generated) of one element");
		auto const ga = GlobalAddress::from_global(AddressSpace::extmem.to_global_omnibus() + next_offset);
		next_offset += 0x100;
	...
	{
		libnux_testcase_begin("Vector (fxvoutx) write to extmem (fxviox address), scalar read (generated) of one element");
		auto const ga = GlobalAddress::from_global(AddressSpace::extmem.to_global_omnibus() + next_offset);
		next_offset += 0x100;
