...
from dlens_vx_v1.sta import PlaybackProgramBuilder, \
    generate, DigitalInit
from dlens_vx_v1.hal import PPUMemoryWord, Timer
from dlens_vx_v1.halco import iter_all, PPUOnDLS
from dlens_vx_v1.tools.run_ppu_program import load_and_start_program, \
    stop_program
import dlens_vx_v1.halco as halco
import dlens_vx_v1.sta as sta
from pylola_vx_v1 import PPUElfFile
...
class LibnuxBarrierTest(unittest.TestCase):
    ...
    def test(self):
        ...
        """
        path = join(TEST_BINARY_PATH, "barrier_two_ppus_vx_v1.bin")
        ...
        builder.wait_until(halco.TimerOnDLS(), int(1e8))
        sta.run(self.CONNECTION, builder.done())
        for ppu in iter_all(PPUOnDLS):
